...
CREATE TABLE IF NOT EXISTS `tickets` (
  ...
  `pending_admin_guidance` TINYINT(1) NOT NULL DEFAULT 0,
  `guidance_requested_by` INT DEFAULT NULL,
  `guidance_requested_at` DATETIME DEFAULT NULL,
  `guidance_comment` TEXT DEFAULT NULL,
  ...
  FOREIGN KEY (`guidance_requested_by`) REFERENCES `users` (`id`) ON DELETE SET NULL,
...
-- Ticket cost revision history for financial reporting and audit
...
-- External work logs (off-platform client requests)
